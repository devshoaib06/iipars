...
class Manage_image_category extends CI_Controller
{
  ...
  $admin_details=$this->session_check_and_session_data->admin_session_data();
    ...
    function video_gallery_view()
    {
        ...
        $data['active']="Manage_image_category";

        $data['vedio_gallery_list']=$this->admin_model->selectAll('tbl_image_gallery');

        //echo "<pre>";print_r($data['examination_list']);exit;
       
        $this->load->view('common/header');
        $this->load->view('common/leftmenu',$data);
        $this->load->view('image_gallery/image_gallery_view',$data);
        $this->load->view('common/footer');
    }
    ...
    function add_video_gallery()
    {
        ...
        $this->load->view('common/header');
        $this->load->view('common/leftmenu',$data);
        $this->load->view('image_gallery/add_vedio_gallery',$data);
        $this->load->view('common/footer');
    }
    ...
    function add_video_gallery_action()
    {
        ...
        else
        {
           ...
           /* if ($ext == "png" || $ext == "gif" || $ext == "jpg" || $ext == "jpeg")
            {*/
                ...
                $config['new_image'] ='../assets/upload/gallery_image/resize/' . $image;
        ...
        $this->db->insert('tbl_image_gallery',$data);
        $this->session->set_flashdata('message','Data Successfully Submitted.');
        redirect('manage_image_category/video_gallery_view','refresh');
    ...
    function video_gallery_edit()
    {
        ...
        $data['edit_details'] = $this->common->select($table_name='tbl_image_gallery',$field=array(), $where=array('video_gallery_id'=>$video_gallery_id), $where_or=array(),$like=array(),$like_or=array(),$order=array(),$start='',$end='');
        ...
        $this->load->view('common/header');
        $this->load->view('common/leftmenu',$data);
        $this->load->view('image_gallery/edit_video_gallery',$data);
        $this->load->view('common/footer');
    ...
    function edit_video_gallery_action()
    {
        ...
        else
        {
            ...
            $ext = substr(strrchr($file, '.'), 1);
             ...
             $hidden_image=$this->input->post('hidden_image');
                ...
                @unlink('../assets/upload/gallery_image/resize/'.$hidden_image);
        ...
        $this->db->update('tbl_image_gallery',$data);
    ...
    function delete_data()
    {
        ...
        $data = $this->common->select($table_name='tbl_image_gallery',$field=array(), $where=array('video_gallery_id'=>$video_gallery_id), $where_or=array(),$like=array(),$like_or=array(),$order=array(),$start='',$end='');
        if(count($data)!=0)
        {
            $image=@$data[0]->video_gallery_image;

             @unlink('../assets/upload/gallery_image/'.$image);
             @unlink('../assets/upload/gallery_image/resize/'.$image);
    ...
    function change_to_active_video_gallery()
    {
        ...
        for($i=0;$i<count($pro_ids);$i++)
        {
            ...
            if($this->db->update('tbl_image_gallery',$data))
            {
                $result=1;
            }
            else
