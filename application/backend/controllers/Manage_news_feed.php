...
class manage_news_feed extends CI_Controller 
{
	...
	public function index()
	{
		...
		if($this->session->userdata('hs_admin_id')=="")
      {
        ...
        $data['active']="news_feed";

        $data['news_feed']=$this->admin_model->selectAll('tbl_news_feed');

		$this->load->view('common/header');
        $this->load->view('common/leftmenu',$data);
        $this->load->view('news_feed_list',$data);
	 ...
	 function insert()
    {
    	...
    	$feed_link=$this->input->post('feed_link');
        ...
        else
        {
            ...
            if ($ext == "png" || $ext == "gif" || $ext == "jpg" || $ext == "jpeg" || $ext == "pdf")
            {
                ...
                move_uploaded_file($file_tmp, "../assets/upload/news_feed/" . $new_name . "." . $ext);
                $image = $new_name . "." . $ext;

                $config['image_library'] = 'gd2';
                $config['source_image'] = '../assets/upload/news_feed/' . $image;
                ...
                // $config['maintain_ratio'] = FALSE;
                // $config['new_image'] = '../assets/upload/why_choose_us/' .$image;
                // $config['quality'] = "100%";
                // $config['width'] = 512;
                // $config['height']= 512;
            ...
            //     redirect(base_url().'index.php/manage_news_feed');
            // }
        }
    	$data = array(
                        ...
                        'title'=>$first_title,
    					'description'=>$feed_link,
                        'image'=>$image,
    					'created_date'=>$current_date,
                        'created_by'=>$id   					
    				);
    	
    	$this->db->insert('tbl_news_feed',$data);
        
    	$this->session->set_flashdata('succ_msg','added successfully');
    	redirect('manage_news_feed','refresh');
    ...
    function edit()
    {
		...
		$data['news_feed']=$this->admin_model->selectOne('tbl_news_feed','id',$edit_id);
		 $data['active']="news_feed";

        $this->load->view('common/header');
        $this->load->view('common/leftmenu',$data);
		$this->load->view('news_feed_edit',$data);
        $this->load->view('common/footer');
    ...
    function edit_action()
    {
        ...
        else
        {
		  ...
		  {
            @unlink('../assets/upload/news_feed/'.$old_img);
       ...
       // print_r($data);exit;
       $this->db->where('id',$id);
       $this->db->update('tbl_news_feed',$data);
    ...
    function delete()
    {
    	...
    	$image_find = $this->admin_model->selectOne('tbl_news_feed','id',$del_id);
	    $old_image = $image_find[0]->image;
	    //echo $old_image; exit;
	    if($old_image!='')
	    {
        @unlink('../assets/upload/news_feed/'.$old_image);
		// @unlink('../assets/upload/slider/small/'.$old_image);
	    }
	   $this->db->where('id',$del_id);
	   $this->db->delete('tbl_news_feed');			
    ...
    function change_slider_status()
{
		...
		for($i=0;$i<count($test_ids);$i++)
		{
			...
			if($this->db->update('tbl_news_feed',$data))
			{
				$result=1;
			}
