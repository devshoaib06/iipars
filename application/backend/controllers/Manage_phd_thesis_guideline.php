...
class manage_phd_thesis_guideline extends CI_Controller 
{
 ...
 //        $this->load->view('common/footer');
	 ...
	 function insert()
    {
        ...
        $this->db->where('id',1);
    	$this->db->update('tbl_phd_thesis_guideline',$data);
    	$this->session->set_flashdata('succ_msg','Updated successfully');
    	redirect('manage_phd_thesis_guideline','refresh');
    ...
    function phd_thesis_online_application()
    {

        $data['php_thesis_online']=$this->admin_model->selectAll('tbl_phd_thesis_form');
         


        $this->load->view('common/header');
        $this->load->view('common/leftmenu',$data);
        $this->load->view('phd_thesis_guideline/php_thesis_online',$data);
        $this->load->view('common/footer');
    }
     ...
     function export_csv_ebook_list()
  {
    $enquiry_id=$this->uri->segment('3');
      header("Content-type: text/csv");
          ...
          header("Content-Disposition: attachment; filename=phd thesis form.csv");

          $marks_details= $this->common->select($table_name='tbl_phd_thesis_form',$field=array(),$where=array('id'=>$enquiry_id),$where_or=array(),$like=array(),$like_or=array(),$order=array(),$start='',$end='',$where_in_array=array());
// echo "<pre>";print_r($marks_details);die;
          $output = fopen('php://output', 'w');

          fputcsv($output, array('Sl No.','Name','Designation','Phone','Email Id','Whatsapp No.','Address','Subject','Thesis Title','Short Description'));
    ...
    function delete()
    {
       ...
       $this->db->delete('tbl_phd_thesis_form');            
       $this->session->set_flashdata('succ_msg','deleted successfully');
            
            
    redirect('manage_phd_thesis_guideline/phd_thesis_online_application','refresh');
    }
