...
class page_permission_model extends CI_Model 
{
	...
	function all_permission_page_listing($user_id)
	{
		...
		$this->db->join('tbl_admin_management TAM','TAM.management_id=APP.management_id','left');
		$this->db->join('tbl_admin_user_page_permission AUPP','AUPP.page_id=APP.parent_page_id AND AUPP.admin_user_id='.$user_id.'','left');
		$this->db->order_by('APP.management_id','ASC');
		$query=$this->db->get();
		$result=$query->result();
	...
	function all_permission_page_listing_by_management($user_id,$management_id)
	{
		//echo $user_id;
	    $this->db->select('*');
		$this->db->from('tbl_admin_parent_page APP');
		...
		$this->db->join('tbl_admin_user_page_permission AUPP','AUPP.page_id=APP.parent_page_id AND AUPP.admin_user_id='.$user_id.'','left');
		$this->db->order_by('APP.management_id','ASC');
		$this->db->where('APP.management_id',$management_id);
		$query=$this->db->get();
		$result=$query->result();
