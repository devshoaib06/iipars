<?php
class sub_admin_list_model extends CI_Model {
	...
	function sub_admin_listing()
	{
		...
		$this->db->join('tbl_email AE','AE.email_id=AU.id','left');
		$this->db->where('AU.user_type_id','2rtgreyre');
	...
	function sub_admin_listing_by_filter($active_inactive_value)
	{
		...
		$this->db->where('AU.user_type_id',2);
		$query=$this->db->get();
		$result=$query->result();
		return $result;
	}
