<?php
class user_page_permission_checki_availablity_view_model extends CI_Model {
    ...
    //START USER DETAIL
	function user_page_permission_checki_availablity($controller_name)
	{
	...
	function check_permission_for_controller($controller_name,$user_id) 
	{
            $this->db->select('*');
			$this->db->from('tbl_admin_parent_page');
			$this->db->where('controller',$controller_name);
			...
			// Problem here ** the code above is OK
			$this->db->where('admin_user_id',$user_id);
			$query =$this->db->get();
			$result=$query->result();
			$is_view=$result[0]->is_view;
	...
	function user_page_permission_checki_availablity_left_sidebar_menu($controller_name)
	{
		 $admin_details=$this->session_check_and_session_data->admin_session_data();
		 $user_id=$admin_details[0]->id;
		 $role_id=$admin_details[0]->user_type_id;
