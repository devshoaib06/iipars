...
{	
	...
	public function index()
	{
		...
		$user_details = $this->common_model->common($table_name='tbl_user',$field=array(),$where=array('user_id'=>$user_id),$where_or=array(),$like=array(),$like_or=array(),$order=array(),$start='',$end='',$where_in_array=array());
		...
		if($user_type=='2')
		{
			...
			$cart_session = $this->session->userdata('cart_session_id');
			$user_id = $this->session->userdata('user_session_id');
			$this->db->where('cart_session_id',$cart_session);
	...
	public function order_submit_action()
	{
		...
		if(@$user_details[0]->user_type_id=='3')
		{
		//echo '<pre>'; print_r($this->session->all_userdata()) ; exit;
				...
				foreach($cart_list as $cart)
		          {
		          	...
		          	if(@$product_details[0]->user_type_id=='2')
		          	{
				...
				$order_details = $this->common_model->common($table_name = 'tbl_order_details', $field = array(), $where = array('order_id'=>$order_id), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		        ...
		        $admin_details_array= $this->common_model->common($table_name = 'tbl_email', $field = array(), $where = array('email_id'=>1), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
				...
				array(
					...
					$html_email_user = $this->load->view('mail_template/user_invoice_template',$email_data, true);

					$html_email_admin=$this->load->view('mail_template/admin_invoice_template',$email_data, true);
					...
					@$this->email->subject('New Order has been Placed | DSY Apnabazer');
					@$this->email->message($html_email_admin);
					@$this->email->send();

					/* -- Send to Vendor -- */
					@$this->email->from($admin_from,'DSY Apnabazer');
					@$this->email->to($email);
					@$this->email->subject('Your Order has been Placed | DSY Apnabazer');
					...
					$this->session->unset_userdata('cart_session_id');
		          	...
		          	$this->session->unset_userdata('checkout_total_amount');
				...
				//---------------------------------------------------------------------------------//
					...
					else
					{
						...
						redirect($this->url->link(24), 'refresh');
