...
{	
	...
	function registration_submit()
	{
		...
		$c_password=trim($this->input->post('c_password'));
		 ...
		 $email_check=$this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('login_email'=>$email,'user_type_id'=>'3'), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		 ...
		 else
		 {
				...
				//******************************** Mail Sent To customer******************************************//
		           ...
		           $data['email']=$this->common_model->common($table_name = 'tblemail', $field = array(), $where = array('email_id'=>1), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		            ...
		            $this->email->subject('Registration With us mail from AKD Ecommerce');
		            $this->email->message($html_email_user);
		            @$reponse=$this->email->send();

		            $this->email->from($admin_from);
		            $this->email->to($email);
		            $this->email->subject('Registration With us Reply from  AKD Ecommerce');
	...
	public function login_action()
	{
		...
		$login_avail=  $this->common_model->common($table_name = 'tbl_user', $field = array(), $where = array('login_email'=>$username,'login_pass'=>$password), $where_or = array(), $like = array(), $like_or = array(), $order = array(), $start = '', $end = '');
		...
		if(count($login_avail) >0 || count($login_avail2) >0)
		{
	...
	public function session_set($login_avail)
		{
			 ...
			 $this->session->set_userdata($drroy_log_session);
                    ...
                    $admin_from=$data['email'][0]->from_email;
                     ...
                     $email_data['mail_data']=array
                       ...
                       $this->email->set_mailtype("html");  
			            ...
			            $this->email->subject('Reset Password Mail From Dr.roy Ecommerce');
			            $this->email->message($html_email_user);
         			    @$reponse=$this->email->send();

         			    $this->session->set_flashdata('pass_succ_msg','Please Check!!! Your password has been sent successfully to your registered Email id....');
         			    redirect($this->url->link(3),'refresh');                
