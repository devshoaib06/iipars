...
class QuestionsController extends Controller
{
    ...
    public function ajaxMockQuestionList(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $arrSearch[] = ['mock_question_master.id', '<>', 0];
            ...
            if ($paper_id != '') {
                $arrSearch[] = ['mqmd.paper_id', $paper_id];
            }
            if ($option_type != '') {
                $arrSearch[] = ['option_type', $option_type];
            }
            DB::connection()->enableQueryLog();
            $items = MockQuestionMaster::select('mock_question_master.*','mqmd.paper_id')->
            join('mock_question_master_details as mqmd','mqmd.question_id','mock_question_master.id')
            ->distinct('mock_question_master.id')
            ->where($arrSearch);
            // $items = MockQuestionMaster::whereHas('questionDetails', function ($q) use ($arrQDetailsSearch) {
            //     $q->select('mock_question_master.id','mock_question_master.is_active');
            ...
            $items_1 =MockQuestionMaster::select('mock_question_master.*','mqmd.paper_id')->
            join('mock_question_master_details as mqmd','mqmd.question_id','mock_question_master.id')
            ->distinct('mock_question_master.id')
            ->where($arrSearch);
