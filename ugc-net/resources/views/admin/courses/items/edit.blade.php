                        ...
                        <div class="portlet light portlet-fit portlet-datatable bordered">
                            <div class="portlet-title">
                                <div class="caption">
                                    ...
                                    <span class="caption-subject font-dark sbold uppercase"> Edit item
                                    </span>
                                </div>
                            ...
                            <div class="portlet-body">
							     <form role="form" action="{{route('editItemAction',['id'=>\Hasher::encode($material_item->item_id)])}}" id="material_info_form" method="POST">
                                            {{ csrf_field() }} 
                                ...
                                <div class="tabbable-line">
                                    ...
                                    <div class="tab-content">
                                                    ...
                                                    <div class="tab-content">
                                                        <div id="tab_1-1" class="tab-pane active">
                                                                ...
                                                                <div class="form-group">
                                                                    ...
                                                                    <input type="text"  name="g_name" class="form-control" value="{{$material_item->name}}"/> 
                                                                </div>
																<div class="form-group">
                                                                    <label class="ccontrol-label">Status <span class="required"> * </span></label>
                                                                    <div class="radio-list">
                                                                        <label class="radio-inline" for="example-inline-radio1">
                                                                            <input id="example-inline-radio1" type="radio" value="1" name="status" {{$material_item->status==1?'checked':''}}> Active</label>
                                                                        <label class="radio-inline" for="example-inline-radio2">
                                                                            <input id="example-inline-radio2" type="radio" value="0" name="status" {{$material_item->status==0?'checked':''}}> Inactive</label>
                                                                    </div>
                                                                </div> 
                                        ...
                                        <div class="tab-pane" id="tab_2">
                                            ...
                                            <table class="table table-striped table-bordered table-hover" id="employer_list">
                                                <thead>
                                                    ...
                                                    @foreach($items as $item)
                                                        @if($item->media_type=='pdf' && $item->value!="" )
                                                            <tr role="row" class="filter">
                                                                <td>{{$i}}</td>
                                                                <td><a href="{{route('downloadItem',['media_id'=>$item->media_id])}}">{{$item->value}}</a></td>
                                                                <td>
                                                                    <button class="btn btn-sm red delete-btn" data-id={{$item->media_id}}>
                                                                        <i class="fa fa-times"></i> Remove</button>
                                                                </td>
                                                            </tr>
                                                      <?php $i++;?>
                                                        @else
                                                            <?php if($not_found==1) { $not_found=0; ?>
                                                            <tr>
                                            ...
                                            <input type="hidden" name="delete_materials[]"  id="delete_materials" />
                                        </div>
                                        ...
                                        <div class="tab-pane" id="tab_3">
                                            ...
                                            @foreach($items as $item)
                                                ...
                                                <div class="row">
                                                <div class="form-group col-sm-6 p-rel">
                                                      <input type="text" placeholder="Video Url" name="videos[]" value="{{$item->value}}" class="form-control" />
                                                      <a href="javascript:;" data-id="{{$item->media_id}}" data-repeater-delete class="btn btn-danger delete-btn p-art"><i class="fa fa-close"></i></a>
                                                 </div>
                                                </div>
                                                @endif
                                            @endforeach  
                                             ...
                                             <div class="form-group">
                                                ...
                                                <div class="checkbox-list">
                                                    @foreach($contributors as $contributor)
                                                        <label>
                                                            <input type="checkbox" name="contibutors[]" value="{{$contributor->contributor_id}}"
                                                            {{in_array($contributor->contributor_id,$s_contributors)?'checked':''}}
                                                            >{{$contributor->name}}  
                                                        </label>
                                                    @endforeach
...
<script type="text/javascript">	
    ...
    $('#add').click(function(){  
           ...
           $('#dynamic_field').append('<div class="row"><div class="form-group col-sm-6 p-rel"><div id="row'+i+'" class="dynamic-added"><input type="text" name="videos[]" placeholder="Video Url" class="form-control" /><a href="javascript:;" id="'+i+'" data-repeater-delete class="btn btn-danger btn_remove p-art"><i class="fa fa-close"></i></a></div></div></div>');  
      }); 
    ...
    jQuery(document).ready(function(){
        $('.delete-btn').click(function(){
            ...
            let prevalue=$("#delete_materials").val();
            let finalDelVal=prevalue;
                finalDelVal+=material_id+',';
            $("#delete_materials").val(finalDelVal);

            $(this).closest('tr').remove();
            $(this).parent('.p-rel').remove();
        });
    });
</script>
