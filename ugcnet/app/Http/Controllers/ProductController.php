...
class ProductController extends Controller
{
        ...
        if (Auth::guard('admins')->check()) {
            try {
                ...
                try {
                    ...
                    // if($product_info && $request->has('item_id')){
                    //     $product_id=$product_info->product_id;
                    //     $this->saveProductContributorItem($request,$product_id);
                    // }
    ...
    private function saveCourseSubject($request,$product_id){
        ...
        try{
            ...
            //     CourseSubjectRelation::updateOrCreate($product_item_data,$product_item_data);
            // }
            return;
        }catch (\Exception $e) {
        ...
        if ($request->ajax())
        {
            ...
            if($subject_id !='')
            {
                $items = DB::table('products')
                           ->select('products.name','products.start_date','products.end_date', 'products.product_id', 'products.status',
                                    'cepr.exam_id','csr.subject_id','products.updated_at')
            ...
            else
            {
                ...
                $items = DB::table('products')
                ->select('products.name','products.start_date','products.end_date', 'products.product_id', 'products.status',
                        'cepr.exam_id','cepr.paper_id','products.updated_at')
                ->leftJoin('course_exam_paper_relations as cepr','products.product_id','cepr.product_id')
                ->groupBy('products.product_id')
                           ->where($arrSearch);
                //echo "sfsf";           
            ...
            if($subject_id !='')
            {
                $items_1 = DB::table('products')
                           ->select('products.name','products.start_date','products.end_date', 'products.product_id', 'products.status',
                                    'cepr.exam_id','cepr.paper_id','csr.subject_id','products.updated_at')
                           ->leftJoin('course_exam_paper_relations as cepr','products.product_id','cepr.product_id')
                           ->leftJoin('course_subject_relations as csr','products.product_id','csr.product_id')
                           ->groupBy('products.product_id')
                           ->where($arrSearch);
        ...
        if (Auth::guard('admins')->check())
        {
            ...
            if($course_infos){
                ...
                foreach($paper_allmaterial as $paper=>$allmaterial){
                        ...
                        // $all_material_details=array(
                        //     'allmaterial'=>$allmaterial[0],
                        //     'paper_id'=>$paper_id,
                        // );
