						...
						<div class="portlet light portlet-fit portlet-datatable bordered">
							<div class="portlet-title">
								<div class="caption">
									...
									<span class="caption-subject font-dark sbold uppercase"> Request from - {{@$order_info[0]->name}}
                    
										<span class="hidden-xs">| {{\Carbon\Carbon::parse(@$order_info[0]->requested_date)->format('F j, Y')}}  </span>
									</span>
								</div>
													...
													<div class="portlet grey-cascade box">
														...
														<div class="portlet-body">
															<div class="table-responsive">
																<form action="{{route('updatePaymentRequestDetails',['id'=>\Hasher::encode(@$order_info[0]->id)])}}" method="POST" id="request_info_form">
																	@csrf
																		...
																		@forelse ($order_info as $order)
																			...
																			<td>
																				<input type="checkbox"  class="form-control pay-checkbox" 
																					value="{{$order->order_id}}" name="order_no[]"
																					{{$order->pay_status==2?'disabled':''}}
																				>
																			</td>
																			<td>{{$order->order_no}}</td>
																			<td>{{$order->course_name}}</td>
																			<td>{{\Carbon\Carbon::parse($order->purchase_date)->format('d/m/Y')}}</td>
																			<td>{{floor($order->revised_price?$order->revised_price:$order->price)}}</td>
																			<td>{{$order->order_amount}}</td>
																			<td>{{$pay_status}}</td>
																			@if($order->pay_status==2)
																				<td>
																					{{\Carbon\Carbon::parse($order->pay_date)->format('d/m/Y')}}
																				</td>
																				<td>
																					@if($order->pay_type==1)
																					Cheque
